...
Tags:  wordpress slider plugin, responsive slider, wordpress slider, custom post slider
...
= 3.1.7 = 
* Solved a few compatibility issues with WordPress 4.7.

= 3.1.6 =
* Solved a bug that on some ocasions prevented WP redirect when saving a slider by issuing several undefined index notices as reported [here](https://wordpress.org/support/topic/not-working-1478/ "Bug report"). 

= 3.1.5 =
* Solved a bug that was displaying the slider even if no content was found.

= 3.1.4 =
* Implemented alternate text for slider images to make sliders ASA compliant. By default alternate text will be taken from the media image.
* Implemented a new filter "the_fa_image_alt_text" that allows developers to override the default media alternate text. Passes as parameters the attachment image ID, the slide post ID and the slider post ID.

= 3.1.3 = 
* Solved taxonomy pagination bug when choosing slider content
* Renamed text domain to "featured-articles-lite" to allow translations
* Implemented 2 new filters: _fa_slider_posts_query_args and _fa_slider_mixed_content_query_args that can be used to modify slides query
* If slider theme can't be found when displaying a slider, if current user is admin, an error will be displayed to signal that the theme is missing 
